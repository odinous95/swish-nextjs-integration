...
import { extraProteinBars } from "@/global-ui/meals/MealData";

interface ProteinBarModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (id: string) => void;
}

export function ProteinBarModal({ isOpen, onClose, onAdd }: ProteinBarModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      extraProteinBars.forEach((bar) => {
        initialQty[bar.id] = 0;
        initialAdded[bar.id] = false;
      });
  ...
  // Lägg till en bar i varukorgen utan att stänga modalen
  ...
  // Kolla om minst en bar har qty > 0 eller är tillagd
  const hasAny =
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-xl p-6 w-full max-w-lg mx-auto my-4 max-h-[90vh] overflow-y-auto flex flex-col">
        ...
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold text-black flex-1 text-center">
            Protein Bars
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          {extraProteinBars.map((bar) => {
            const qty = quantities[bar.id] || 0;
            const isAdded = added[bar.id];
            return (
              <div
                key={bar.id}
              ...
              >
                ...
                <div className="flex items-center gap-4 mb-3">
                  <button
                    onClick={() => adjustQuantity(bar.id, -1)}
                    disabled={qty === 0}
